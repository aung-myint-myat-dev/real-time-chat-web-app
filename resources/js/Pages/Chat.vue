...
const markConversationAsRead = (messageId = null) => {
    ...
    axios.post(`/chats/${props.conversation.id}/read`, { message_id: messageId });
};
...
<template>
    <div v-if="props.conversation" class="h-screen overflow-hidden flex flex-col">
        ...
        <div ref="messagesContainer" class="flex-1 overflow-y-auto p-4 relative" @scroll="handleScroll">
        ...
        <div class="border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 shrink-0">
            <form @submit.prevent="handleSendMessage" class="flex items-end gap-3">
                <!-- Textarea -->
                <div class="flex-1">
                    <textarea v-model="message" rows="1" placeholder="Type a message..."
                        ...
                        @keydown.enter.exact.prevent="handleSendMessage" @keydown.shift.enter.stop
                        @keydown="sendTypingEvent"></textarea>
                </div>
